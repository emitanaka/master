...
```{r gender,fig.dim=c(4.8, 4.5), out.width="100%", dev='svg'}
ggplot(df, aes(gender, frequency)) + geom_point() + stat_smooth(aes(group = 1), method = "lm", se = FALSE)
```
]
...
```{r subject, fig.height=5, dev='svg'}
...
```{r scenario, fig.height=5, dev='svg'}
