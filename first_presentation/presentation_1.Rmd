...
qplot(x = index, y = resm_std, color = subject, data = fit_df, geom = "point") %+% 
  geom_hline(yintercept = 0) %+% 
  lineup(true = fit_df, samples = simdat)+ 
...
# Lineups
```{r lineup_2_2, fig.height=6, fig.width=10, dev='svg'}
qplot(x = index, y = resm_std, color = scenario, data = fit_df, geom = "point") %+% 
  geom_hline(yintercept = 0) %+% 
  lineup(true = fit_df, samples = simdat)+ 
  facet_wrap( ~ .sample, ncol=5) +
  xlab(NULL) + ylab(NULL) + 
