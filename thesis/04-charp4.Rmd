...
Position of the data plot may also matters.

If we can add the tolerance interval/area for QQ-plots and residual plots, it can gives us more benefits on the result since it's better in reflecting the null distribution for a specific diagnostic feature and improving objectivity for interpreting these plots.

Since we colouring the residual plot by different subjects, then many observers are looking for the cluster pattern rather than the outlying observations. If we add one more sentence such as our project is focus on the residual analysis, it will have a better understanding for them to choose the plots.
