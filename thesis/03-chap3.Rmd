...
The Rorschach is taken as the cognitive experimentation. To measure a data analyst's tendency to over interpret plots in which there is no or only obvious structure is the goal. Although it can be bias based on the analysts' knowledge, the aim of this training is to improve the awareness of the features when they detect. 
...
Even if the observers has no idea of the distinguishable features, they can still select one based on their feelings. But at most case, the viewers select the response based on the identifying features compared to the rest (@buja2009statistical). @buja2009statistical also carries out the characteristic of lineup that it can be made to choose the rank of the difference level of the plots. If this protocol is repeated to multiple independent observers, the $p$-value will be realized by tabulating the number of independent investigators chosen the data plot among the 19 null plots. Afterwards, the definition of $p$-values of lineup is implemented by @buja2009statistical and @majumder2013validation. For $K$ independent investigators, let $X$ be the number of observers picking the test statistic from the lineup. Under the null hypothesis, $X$ is distributed as Binomial distribution $\text{Binom}_{K, 1/m}$ (for a lineup of size $m$). Hence, @majumder2013validation introduce the visual $p$-value of a lineup of a size $m$ evaluated by $K$ observers is given by
$$P(X \geq x) = 1 - Binom_{K, 1/m}(x-1)$$
with $x$ is the number of the observers pointing out the actual data plot and the reason for $(x-1)$ is that for binomial distribution, it only takes the integer values. Then with the significance level $\alpha$, the author indicates decision rule that we can reject the null if out of $K$ observers at least $\chi_\alpha$ correctly identify the data plot, otherwise, fail to reject the null. While the lineup size will affect the probability that the viewer correctly points the data plot out proved by @majumder2013validation, such that with larger $m$, the probability of correctly distinguishing the data plot decreases. However, we often choose the lineup size as 20.

They always run the human subject experiment through @turk2012amazon. The demographic information of individuals will be asked, such as age, gender, education level etc. Furthermore, the time taken to responses for each observer is recorded and observers are asked to give the confidence level from 1 to 5 with weak to high (@majumder2013validation, @loy2017model). In order to test the ability of individuals, @majumder2013validation gives the easy lineup as the reference lineup. If they are able to identify the actual data plot, then the response for the reference lineup will be removed and all the following responses will be stored. If the answer for the reference lineup is wrong, then the all responses made by this observer will be removed.
