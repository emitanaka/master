...
ggplot(lin.fitted, aes(interaction(attitude, gender), resc_std)) + 
  geom_hline(yintercept = 0) +
  geom_boxplot() 
```
...
```{r v11n-plot21}
lineup(true = lin.fitted, samples = lin.sim.fitted, pos = 9) %>% 
...
```{r}
ggplot(v2.fitted, aes(index, resm_std, color = subject)) + geom_point()
ggplot(v2.fitted, aes(index, resc_std, color = subject)) + geom_point()
