...
```{r}
Bryk <- as.data.frame(MathAchieve[,c("School","Sex", "Minority", "SES", "MathAch")])
...
```{r}
ggplot(Bryk, aes(Sector, MathAch)) + geom_boxplot()
```
...
ggplot(MathAchieve, aes(Sex, MathAch)) +
  geom_boxplot() + facet_grid(.~Minority)
```
...
```{r}
model <- lmer(MathAch ~ Sex + SES + (1|School), data = df)
summary(model)
```

```{r}
fit <- lmer(MathAch ~ SES + Sector + Minority + Sex + (1|School), data = Bryk)
...
y <-  Bryk$MathAch
X <- model.matrix(~ SES + Sector + Minority + Sex, data = Bryk)
beta <- fixef(fit)
