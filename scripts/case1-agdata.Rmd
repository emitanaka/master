...
simdat <- purrr::map_dfr(1:nsim, function(i){
  ...
  simfit <- mmer(ysim ~ 1 + loc,
                 ...
                 rcov =~units,
                 data = simdat)
  tibble::tibble(.n = i, simfit, structure(simfit, "list"), fitted = fitted(simfit) ,std.resm, std.resc)
})
...
true.df <- data.frame(true, std.resm, std.resc,fitted = fitted(fit), index = seq.int(nrow(true.df)))
```
...
```{r}
qplot(x = index, y = std.resm, data = true.df, geom = "point") %+% 
  lineup(true = true.df, samples = simdat) + 
  facet_wrap( ~ .sample, ncol=5) + 
  xlab(NULL) + ylab(NULL) +
  theme(axis.text.y = element_blank(), axis.text.x = element_blank(),
...
```{r lineup_plot5}
qplot(x = fitted, y = std.resc, data = true.df, geom = "point") %+% 
  lineup(true = true.df, samples = simdat) + 
  facet_wrap( ~ .sample, ncol=5) + 
  xlab(NULL) + ylab(NULL) +
  theme(axis.text.y = element_blank(), axis.text.x = element_blank(),
