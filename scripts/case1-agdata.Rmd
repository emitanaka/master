...
```{r Not the correct one}
# simulate y for 19 times
set.seed(12345)
y.sim <- list()

for (i in (1:19)){
  y.sim[[i]] <- unlist(as.vector(MASS::mvrnorm(n = 1, mu = X %*% beta_hat, Sigma = V)))
  twmp <- lapply(y.sim, function(y) 
                 cbind(y, loc = df$loc, gen = df$gen))
}

# not include the true data set in the null
# twmp_new <- append(twmp, list(cbind(y = df$yield, loc = df$loc, gen = df$gen)))
```
...
lapply(1:nsim, function(i){
  ysim <- data.frame(y = MASS::mvrnorm(n = 1, mu = X %*% beta_hat, Sigma = V))
  simdat <- cbind(ysim, loc = as.factor(df$loc), gen = as.factor(df$gen)) 
  ...
  out_df <- data.frame(simdat, std.resm, fitted = fitted(simfit), sample = i)
})

# this is not i wanted FAIL!!!
for (i in 1:nsim){
  ysim <- data.frame(y = MASS::mvrnorm(n = 1, mu = X %*% beta_hat, Sigma = V))
  simdat <- cbind(ysim, loc = as.factor(df$loc), gen = as.factor(df$gen)) 
  ...
  out_df <- data.frame(simdat, fitted = fitted(simfit), sample = i)
}
```
...
true.df <- data.frame(true, std.resm, fitted = fitted(fit))
```

```{r}
qplot(x = fitted, y = std.resm, data = true.df, geom = "point") %+% 
  lineup(true = true.df, samples = out_df)
```
