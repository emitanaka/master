...
ggplot(Blackmore, aes(age, log.exercise, color = group)) + 
  geom_point() + 
  facet_wrap_paginate(~ subject, nrow = 4, ncol = 4, page = 13)
```


```{r}
ggplot(Blackmore, aes(group, log.exercise)) + geom_boxplot()
```
...
## Model 2 without $age^2$ in random effect
...
## Simulate data for null plots based on `fit` model
```{r simulate}
